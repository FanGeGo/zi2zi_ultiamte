...
def draw_example(ch, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes):
    ...
    empty_img= Image.new("RGB", (canvas_size, canvas_size), (255, 255, 255));
    testing_img=Image.new("RGB", (canvas_size * 2, canvas_size), (255, 255, 255));
    testing_img.paste(empty_img, (0, 0))
    testing_img.paste(src_img, (canvas_size, 0))

    example_img = Image.new("RGB", (canvas_size * 2, canvas_size), (255, 255, 255))
    example_img.paste(dst_img, (0, 0))
    example_img.paste(src_img, (canvas_size, 0))
    return example_img, dst_img, src_img, testing_img
...
def font2img(src, dst, charset, char_size, canvas_size,
    ...
    for c in charset:
        if count == sample_count:
            break
        [e,e1,e2,testing_img] = draw_example(c, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes)
        if e:
            #e.save(os.path.join(sample_dir, "%d_%04d.jpg" % (label, count)))
            #e1.save(os.path.join(sample_dir, "%d_%04d.jpg" % (label, count)))
            #e2.save(os.path.join(sample_dir, "%d_%04d_2.jpg" % (label, count)))
            testing_img.save(os.path.join(sample_dir, "%d_%05d.bmp" % (label, count)))
            count += 1
            if count % 100 == 0:
